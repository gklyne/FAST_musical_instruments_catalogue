...
def get_midi_mapping_table(note_table):
    """
    Make table for mapping MEI data to Midi note number.
    Index by octave, note name; e.g.
    ...
    for (midi_num,notename) in note_table:
        note   = note_table[(midi_num,notename)]
        octave = int(note["oct"])
        mapping_table[notename][octave] = int(midi_num)
    return mapping_table

def save_midi_mapping_table(m_data):
    m_text  = json.dumps(m_data, sort_keys=True)
    dirname = "."
    try:
        os.makedirs(dirname)
    except OSError, e:
        if e.errno != errno.EEXIST:
            raise
    print(m_text)
    # filename = os.path.join(dirname, "midi_mapping_table.jsonld")
    # with open(filename, "w") as f:
    #     f.write(m_text)
    return

def main():
    midi_note_data = get_midi_note_data_table()
    midi_mapping_table = get_midi_mapping_table(midi_note_data)
    if midi_mapping_table:
        save_midi_mapping_table(midi_mapping_table)
    return
